  ...
  return (
    <div className="fixed inset-0 z-[118] bg-black/85 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <section
        ...
        className="w-full max-w-[420px] max-h-[88dvh] overflow-y-auto hide-scrollbar rounded-t-[32px] sm:rounded-3xl border border-zinc-800/90 bg-zinc-950 shadow-2xl shadow-black/90 pb-safe"
      >
            ...
            {CAPTURE_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`luxury-feature-card relative min-h-[104px] rounded-2xl border p-3.5 text-left active:scale-[0.97] transition-all ${item.bg}`}
                >
                  <div className="flex justify-between items-start gap-2">
                    <span className={`w-9 h-9 rounded-xl bg-black/40 border border-white/5 flex items-center justify-center shadow-inner ${item.color}`}><Icon size={17} /></span>
                    {complete && <CheckCircle2 size={15} className="text-emerald-400" aria-label="Bugün kaydedildi" />}
                  </div>
                  <strong className="text-[11px] font-black text-zinc-100 block mt-2.5 tracking-tight">{item.label}</strong>
                  <span className="text-[9px] font-mono text-zinc-400 leading-snug block mt-0.5">{complete ? 'Bugün kayıt var · yeniden aç' : item.hint}</span>
                </button>
              );
            })}
          </div>
