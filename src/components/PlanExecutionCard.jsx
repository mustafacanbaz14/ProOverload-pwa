...
const Metric = ({ icon, label, value, detail }) => (
  <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-2.5 min-w-0">
    ...
    {detail && <span className="mt-0.5 block text-[8px] font-mono text-zinc-600 truncate">{detail}</span>}
  </div>
);
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-violet-900/45 bg-zinc-900">
        ...
        <div className="space-y-3 p-3">
          ...
          {report.muscles.length > 0 && (
            <div className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950">
              <div className="flex items-center justify-between border-b border-zinc-800 px-3 py-2.5">
                ...
                <span className="text-[8px] font-mono text-zinc-600">gerçek / plan</span>
              </div>
          ...
          <div className="space-y-1.5">
            ...
            {report.sessions.length === 0 ? (
              <p className="rounded-xl border border-zinc-800 bg-zinc-950 p-3 text-[9px] font-mono text-zinc-600">
              ...
              <div key={session.id || `${session.date}-${session.name}`} className="flex items-center justify-between gap-2 rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2">
                <div className="min-w-0">
                  ...
                  <span className="text-[8px] font-mono text-zinc-600">{formatDay(session.date)}</span>
                </div>
                <div className="shrink-0 text-right">
                  <span className={`block text-[10px] font-mono ${toneForPercent(session.setPercent)}`}>%{session.setPercent}</span>
                  <span className="text-[8px] font-mono text-zinc-600">{session.drift === null ? 'gün ?' : session.drift === 0 ? 'plan gününde' : `${session.drift} gün kaydı`}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {report.weeks.map(week => (
            <div key={week.key} className="px-3.5 py-3">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="w-[68px] text-right text-[8px] font-mono text-zinc-600">
        ...
        <div className="divide-y divide-zinc-800/70">
          {report.templates.map(template => (
            <div key={template.templateId} className="px-3.5 py-3 space-y-1.5">
              ...
              <div className="grid grid-cols-3 gap-1 text-[8px] font-mono text-zinc-600">
      ...
      <div className="border-t border-zinc-800 bg-zinc-950/60 px-4 py-3">
        <div className="flex items-start gap-2">
          <ShieldCheck size={11} className="mt-0.5 shrink-0 text-zinc-600" />
          <p className="text-[8px] font-mono leading-relaxed text-zinc-600">{report.caveat}</p>
        </div>
