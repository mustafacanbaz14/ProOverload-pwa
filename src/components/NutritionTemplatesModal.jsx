        ...
        <main className="flex-1 overflow-y-auto hide-scrollbar p-4 space-y-3.5 pb-safe">
          ...
          {tab === 'recipe' && (
            <section className="rounded-3xl border border-zinc-800/80 bg-zinc-900/60 p-4 space-y-3.5 backdrop-blur-sm shadow-sm">
              ...
              <label className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-950 px-3.5 py-2.5"><span className="text-[10px] font-bold text-zinc-400">Kaç porsiyon?</span><input type="number" min="1" max="20" value={servings} onChange={event => setServings(Math.max(1, Math.min(20, Number(event.target.value) || 1)))} className="w-16 bg-zinc-900 border border-zinc-800 rounded-xl py-1 text-center font-mono text-orange-400 font-bold" /></label>
