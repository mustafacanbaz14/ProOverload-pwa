...
const statusStyle = {
  ...
  unplanned: 'text-zinc-600 border-zinc-800 bg-zinc-950',
};
    ...
    <section className="rounded-2xl border border-violet-900/45 bg-gradient-to-br from-violet-950/20 via-zinc-900 to-zinc-900 overflow-hidden">
      <div className="px-3 py-3 border-b border-zinc-800/80 flex items-start justify-between gap-3">
        <span className="min-w-0">
          ...
          <span className="text-[8px] font-mono text-zinc-600 block mt-1">{profile.personalized} kas kişiselleşti · ortalama güven %{profile.averageConfidence}</span>
        </span>
        <span className="rounded-xl border border-zinc-800 bg-zinc-950 px-2.5 py-2 text-center shrink-0">
          ...
          <span className="text-[7px] font-bold text-zinc-600">BANTTA</span>
        </span>
      </div>
      ...
      {!profile.hasPlan ? (
        ...
        <div className="p-3 space-y-3">
          <div className="grid grid-cols-3 gap-1.5">
            <div className="rounded-xl border border-emerald-900/40 bg-emerald-950/15 py-2 text-center"><strong className="text-sm font-mono text-emerald-300 block">{profile.summary.aligned}</strong><span className="text-[7px] font-bold text-zinc-600">BANTTA</span></div>
            <div className="rounded-xl border border-amber-900/40 bg-amber-950/15 py-2 text-center"><strong className="text-sm font-mono text-amber-300 block">{profile.summary.under}</strong><span className="text-[7px] font-bold text-zinc-600">EKSİK</span></div>
            <div className="rounded-xl border border-red-900/40 bg-red-950/15 py-2 text-center"><strong className="text-sm font-mono text-red-300 block">{profile.summary.lowValueSets}</strong><span className="text-[7px] font-bold text-zinc-600">FAZLA ADAYI</span></div>
          </div>

          <div className="grid grid-cols-4 gap-1">
            {FILTERS.map(item => (
              <button type="button" key={item.key} onClick={() => setFilter(item.key)} className={`rounded-lg border py-1.5 text-[8px] font-bold ${filter === item.key ? 'border-violet-500 bg-violet-950/35 text-violet-200' : 'border-zinc-800 bg-zinc-950 text-zinc-600'}`}>{item.label}</button>
            ))}
          </div>

          <div className="space-y-1.5 max-h-64 overflow-y-auto hide-scrollbar">
            {rows.length === 0 ? (
              <p className="rounded-xl border border-zinc-800 bg-zinc-950 p-3 text-[9px] font-mono text-zinc-600 text-center">Bu süzgeçte kas yok.</p>
            ) : rows.map(row => (
              <button
                type="button"
                key={row.muscle}
              ...
              >
                ...
                <span className="flex items-baseline justify-between gap-2 mt-1 text-[8px] font-mono">
                  ...
                  <span className={row.personalized ? 'text-violet-300' : 'text-zinc-700'}>{row.personalized ? `kişisel %${row.confidence}` : 'referans'}</span>
                </span>
              </button>
            ))}
          </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950 overflow-hidden">
              <div className="p-3 border-b border-zinc-800/80">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[8px] font-mono text-zinc-600">{selected.source}</span>
                </div>
                ...
                <div className="flex justify-between mt-1 text-[8px] font-mono text-zinc-600"><span>plan {selected.planned}</span><span>bant {selected.targetLow}–{selected.targetHigh}</span><span>tavan {selected.base.mrv}</span></div>
              </div>

              <div className="p-3 space-y-2">
                <div className="grid grid-cols-3 gap-1.5">
                  <div className="rounded-lg border border-zinc-800 bg-zinc-900 py-1.5 text-center"><strong className="text-[10px] font-mono text-zinc-300 block">{selected.weeks}</strong><span className="text-[7px] text-zinc-600">kayıtlı hafta</span></div>
                  <div className="rounded-lg border border-zinc-800 bg-zinc-900 py-1.5 text-center"><strong className="text-[10px] font-mono text-zinc-300 block">{selected.evaluatedWeeks}</strong><span className="text-[7px] text-zinc-600">ölçülebilir</span></div>
                  <div className="rounded-lg border border-zinc-800 bg-zinc-900 py-1.5 text-center"><strong className="text-[10px] font-mono text-zinc-300 block">%{selected.confidence}</strong><span className="text-[7px] text-zinc-600">güven</span></div>
                </div>
                ...
                <div>
                  <span className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest flex items-center gap-1 mb-1.5"><CalendarRange size={9} /> 4 haftalık deneme rampası</span>
                  <div className="grid grid-cols-4 gap-1">
                    {selected.ramp.map(week => (
                      <div key={week.week} title={week.rule} className="rounded-lg border border-zinc-800 bg-zinc-900 py-2 text-center">
                        <span className="text-[7px] text-zinc-600 block">H{week.week}</span>
                        <strong className="text-[11px] font-mono text-violet-300 block">{week.sets}</strong>
                        <span className="text-[6px] text-zinc-700 block truncate px-0.5">{week.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {methodOpen && (
            <p className="text-[8px] font-mono text-zinc-600 leading-relaxed px-1">Başlangıç MEV–MAV aralığı; performansın korunduğu tamamlanmış haftaların ortanca ve çeyreklerinden, veri güveni oranında kişiselleştirilir. Eksik hafta başarısız sayılmaz. Toparlanma güçlü/zayıf olsa bile geçici ayar en fazla +1/−2 settir. Bu gözlemsel karar desteğidir; kas büyümesini doğrudan ölçmez.</p>
          )}
        </div>
      )}
    </section>
